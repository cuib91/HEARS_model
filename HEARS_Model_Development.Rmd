...
# ── Prepare matrices ──────────────────────────────────────────────────────────
X_train <- matrix(as.numeric(as.matrix(train_df[, FEAT_NAMES])),
                  nrow = nrow(train_df), ncol = length(FEAT_NAMES))
colnames(X_train) <- FEAT_NAMES
y_train <- as.integer(train_df$RISK_NUM)

X_test  <- matrix(as.numeric(as.matrix(test_df[, FEAT_NAMES])),
                  nrow = nrow(test_df), ncol = length(FEAT_NAMES))
colnames(X_test) <- FEAT_NAMES
y_test  <- as.integer(test_df$RISK_NUM)
y_label_test <- factor(test_df$RISK_LABEL, levels = CLASS_LABELS)

# Verify matrix types — critical for XGBoost and LightGBM
cat("X_train type:", typeof(X_train), "| class:", class(X_train), "\n")
cat("y_train type:", typeof(y_train), "| range:", min(y_train), "-", max(y_train), "\n")
...
for (k in seq_along(folds_xgb)) {
  ...
  xgb_k <- xgb.train(
    params  = xgb_params,
    data    = xgb.DMatrix(data = X_train[tr_idx, , drop=FALSE], label = as.numeric(y_train[tr_idx])),
    nrounds = 100,
    verbose = 0
  )

  raw_k    <- matrix(predict(xgb_k, xgb.DMatrix(data = X_train[val_idx, , drop=FALSE])),
                     ncol = 3, byrow = TRUE)
...
xgb_model_final <- xgb.train(
  params  = xgb_params,
  data    = xgb.DMatrix(data = X_train, label = as.numeric(y_train)),
  nrounds = 100,
  verbose = 0
)
...
# NOTE: lgb.train() v4.6.0 on Apple Silicon (aarch64-darwin) has a known
# multiclass training bug — trains without error but predicts near-randomly.
# Workaround: use xgb.train() with equivalent gradient boosting parameters.
# Python LightGBM results (AUC=0.844, slope=1.001) remain the definitive values.
set.seed(42)
lgbm_equiv_params <- list(
  objective        = "multi:softprob",
  num_class        = 3,
  max_depth        = 5,
  eta              = 0.1,
...
lgbm_model <- xgb.train(
  params  = lgbm_equiv_params,
  data    = xgb.DMatrix(X_train, label = y_train),
  nrounds = 100,
  verbose = 0
)

cat("  LightGBM-equivalent model trained (xgb.train workaround for Apple Silicon)\n")
cat("  Parameters: max_depth=5, learning_rate=0.1 (eta), min_child_weight=20\n")
cat("  Definitive Python LightGBM: AUC=0.844, Cal slope=1.001 (Chapter 4)\n")
```
...
xgb_raw_mat   <- matrix(predict(fit_xgb$model, xgb.DMatrix(data = X_test)),
                        ncol = 3, byrow = TRUE)
...
# LightGBM-equivalent — evaluate using xgb.DMatrix (same as training)
lgbm_raw_mat   <- matrix(
  predict(lgbm_model, xgb.DMatrix(X_test)),
  ncol = 3, byrow = TRUE
)
...
```{r gini-importance}
# Feature importance from LightGBM-equivalent xgb model
lgbm_imp_raw <- xgb.importance(model = lgbm_model)

# Map feature names (xgboost uses f0,f1,...fn notation)
fi_df <- data.frame(
  Feature    = FEAT_NAMES,
  Importance = lgbm_imp_raw$Gain[match(paste0("f", 0:11), lgbm_imp_raw$Feature)]
) %>%
...
xgb.save(lgbm_model, "HEARS_LightGBM_equivalent_final.model")
cat("LightGBM-equivalent model saved: HEARS_LightGBM_equivalent_final.model\n")
cat("(Trained via xgb.train — Apple Silicon lgb.train v4.6.0 multiclass workaround)\n")
